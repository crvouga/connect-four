...
import { head, juxt, last, sortBy } from "ramda";
...
const unDraw = (node) => {
  const length = node.getTotalLength();
  gsap.to(node, 0.5, { strokeDasharray: length, strokeDashoffset: length });
};
...
const Consecutive = (props) => {
  ...
  const [westIndex, eastIndex] = juxt([head, last])(sortBy(head, consecutive));
  ...
  return (
    ...
    >
      ...
      >
        <rect
          transform={`
            rotate(${angleBetween(westIndex, eastIndex)} ${[
            (i + 0.5) * n,
            (j + 0.5) * n,
          ]})
          `}
